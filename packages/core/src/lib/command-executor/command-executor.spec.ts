...
import type { CommandBaseOutputSuccessful } from '../command';
import { CommandContext } from '../command-context';
import type { CommandHandlerAny } from '../command-handler';
import { ServiceAdapterOptions } from '../common-interfaces';
import { RetriesLimitReachedException } from '../exception';

import { CommandExecutor } from './command-executor';

let mockHandler: jest.Mocked<CommandHandlerAny>;
let mockInput: any;
let mockOptions: ServiceAdapterOptions | null;
let mockContext: CommandContext<any>;
const breadId = '1';
...
beforeEach(() => {
  ...
  mockOptions = null;
  mockContext = {} as CommandContext<any>;
});
...
it('should execute successfully without retries', async () => {
  const expectedOutput = { success: true, breadId } as const;
  mockHandler.handle.mockResolvedValue(expectedOutput);

  const retrier = new CommandExecutor({
    handler: mockHandler,
    input: mockInput,
    options: mockOptions,
    context: mockContext,
...
it('should increase delay between retries by two times (by default) the previous delay except the first retry, which should run immediately', async () => {
  ...
  const expectedOutput = {
    success: true,
    breadId,
  } as CommandBaseOutputSuccessful;
...
it(`should retry the operation on retry-eligible HTTP error if handler.shouldRetry returns null`, async () => {
  ...
  } as unknown as CommandHandlerAny;
